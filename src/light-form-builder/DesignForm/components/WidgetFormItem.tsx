...
const WidgetFormItem: FC<Props> = (props) => {
  ...
  const className = useMemo(
    () =>
      `widget-item-container ${selectWidgetItem?.key === key ? 'active' : ''} ${
        ['Row', 'Col', 'Space'].includes(type) ? 'child-nodes' : ''
      }`,
    [selectWidgetItem],
  );
  ...
  if (
    ['DatePicker', 'RangePicker', 'TimePicker'].includes(type) &&
    formItemConfig?.initialValue
  ) {
    if (isString(formItemConfig?.initialValue)) {
      commonFormItemProps.initialValue = moment(
        formItemConfig.initialValue,
        config?.format,
      );
    }
    if (
      isArray(formItemConfig?.initialValue) &&
      formItemConfig.initialValue.length === 2
    ) {
      commonFormItemProps.initialValue = [
        moment(formItemConfig.initialValue[0], config?.format),
        moment(formItemConfig.initialValue[1], config?.format),
      ];
    }
  }
      ...
      const options: Sortable.Options = {
        ...
        setData: (dataTransfer) => {
          dataTransfer.setData(
            'SortableDataMove',
            selectWidgetItemRef.current?.key!,
          );
        },
        onAdd: (event: any) => {
          ...
          const SortableDataClone =
            event.originalEvent.dataTransfer.getData('SortableDataClone');
          const SortableDataMove =
            event.originalEvent.dataTransfer.getData('SortableDataMove');
          ...
          if (SortableDataMove) {
            ...
            const generateNewWidgetFormList = (
              list: Component[],
              key: string,
            ) => {
              ...
              for (let index = 0; index < newList.length; index++) {
                ...
                if (newList[index].childNodes) {
                  newList[index].childNodes = generateNewWidgetFormList(
            ...
            const newWidgetFormList = generateNewWidgetFormList(
              widgetFormList,
              selectWidgetItemKey,
            );
            ...
            dispatch({
              ...
              payload: createNewWidgetFormList(
                newWidgetFormList,
                newChildNodes,
                key!,
              ),
  ...
  const handleCopyClick = (event: MouseEvent<SVGSVGElement>) => {
    ...
    const handleWidgetFormItem = (list: Component[]) => {
      ...
      for (let index = 0; index < newList.length; index++) {
        ...
        if (newList[index].childNodes) {
          newList[index].childNodes = handleWidgetFormItem(
            newList[index].childNodes!,
          );
        }
      }
  ...
  useEffect(() => {
        ...
        fetch(remoteConfig.remoteFunc)
          ...
          .then((json) => {
            if (json instanceof Array) {
              remoteConfig.remoteOptions = handleResponseData(
                json,
                remoteConfig,
              );
              forceUpdate();
    ...
    remoteConfig?.remote,
    remoteConfig?.remoteFunc,
    remoteConfig?.remoteProps.label,
    remoteConfig?.remoteProps.value,
  ]);

  useEffect(
    () => formInstance.resetFields([key!]),
    [selectWidgetItem?.formItemConfig?.initialValue],
  );
      ...
      >
        ...
        {type === 'TreeSelect' && (
          <Form.Item {...commonFormItemProps}>
            <TreeSelect
              {...commonProps}
              treeData={remoteConfig?.remoteOptions}
            />
          </Form.Item>
        )}
        {type === 'Upload' && (
          ...
          >
            {config?.drop ? (
              ...
              <Upload {...commonProps}>
                {commonProps.listType === 'picture-card' ? (
                  '点击上传'
                ) : (
        ...
        {type === 'Calendar' && (
          <Calendar {...commonProps} value={commonProps.defaultValue} />
        )}
